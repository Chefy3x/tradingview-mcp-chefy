...
If the user specifies a different install path, use that instead of `~/tradingview-mcp-chefy`.
...
```bash
cp ~/tradingview-mcp-chefy/rules.example.json ~/tradingview-mcp-chefy/rules.json
```
